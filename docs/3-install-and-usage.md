...
**Примечание.** Для подключения платформенно-зависимых средств используется
один из двух макросов: `R05_POSIX` или `R05_WINDOWS` (но не оба сразу).
Заголовочный файл `refal05rts.h` по предопределённым макросам препроцессора
(например, `_WIN32` или `__unix__`) пытается угадать, под какую операционную
систему выполняется компиляция, и если угадывает — устанавливает
соответствующий макрос. Однако, можно один из этих двух макросов установить
и самостоятельно (например, указав `gcc -DR05_POSIX` в `R05CCOMP`) —
пользовательская настройка перекрывает автоматическую.

В актуальной версии установка этих макросов влияет на возвращаемое значение
функции `System` на POSIX-системах и без указания обоих этих макросов
не определяется функция `GetCurrentDirectory`. Также при установке `R05_POSIX`
на posix-системах быстрее будет выполняться программа с включённым
профилированием (см. далее про `R05_SHOW_STAT_DETAILED` и `R05_PROFILER`).
...
памяти. Например:
    ...
    Total program time: 2.594 seconds.
...
#### Макрос `R05_SHOW_STAT_DETAILED` — детализированный профиль работы программы

Если этот макрос установлен, программа выводит на `stderr` более подробный
отчёт. Например:
...
Рассмотрим каждую из новых строчек подробнее:
...
Если одновременно установлены `R05_SHOW_STAT` и `R05_SHOW_STAT_DETAILED`,
макрос `R05_SHOW_STAT_DETAILED` имеет приоритет.

Вывод подробной статистики может увеличить время выполнения программы примерно
на 5 %. У неподробной статистики таких накладных расходов нет (два вызова
`clock()` в начале и в конце выполнения программы).
...
Вывод профиля может увеличить время выполнения программы примерно на 2 %.
