...
`Xxin` библиотеки `reflib` Рефала-5 VERSION PZ или Version ПѮ.
...
Если на POSIX-системе макрос `R05_POSIX` не установлен, функция `System` будет
возвращать сырое значение функции `system` языка Си, которое может отличаться
от фактического кода возврата (см. `man 2 wait` для более подробных сведений).
...
### 56. GetCurrentDirectory
    ...
    <GetCurrentDirectory> == e.FileName

**Семантика.** Функция возвращает путь к текущей папке.

Функция определена, если установлен макрос `R05_POSIX` или `R05_WINDOWS`, иначе
компилятор будет сообщать, что функция не определена, также она будет
отсутствовать в списке `ListOfBuiltin`.

Функция завершается с ошибкой, если длина пути к текущей папке будет больше,
чем константа `FILENAME_MAX` используемого компилятора Си.
