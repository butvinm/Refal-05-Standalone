...
Это форк компилятора [Рефал-05](https://github.com/Mazdaywik/Refal-05), в котором доработана только сборка.
Компилятор можно развернуть, имея один лишь компилятор C89: сторонний компилятор Рефала-5 для раскрутки не нужен.

В каталоге `bootstrap` лежат C-файлы, сгенерированные предыдущей версией компилятора.
Раскрутка собирает компилятор из них, перегенерирует их из `src/*.ref` и пересобирает компилятор заново.
...
Собранные исполняемые файлы для Unix и Windows публикуются в релизах.
...
Каталог `refal-5-framework` - подмодуль с [Рефал-5-фреймворком](https://github.com/Mazdaywik/refal-5-framework), форку принадлежит только зафиксированная ревизия.
...
* **Простота и инимализм** — в реализации следует избегать сложных конструкций
...
Рекомендуемым компилятором языка Си является Borland C++ Compiler 5.5.1:

<http://altd.embarcadero.com/download/bcppbuilder/freecommandLinetools.exe>

Он компилирует в разы быстрее других известных автору компиляторов и при этом
обеспечивает хорошее быстродействие. Ранее этот компилятор был доступен для
скачивания бесплатно с сайта Embaracero.

**Примечание.** Некоторые провайдеры ошибочно затрудняют скачивание этого
файла: после загрузки ≈30 Кбайт соединение обрывается. Для скачивания файла
можно в цикле перезапускать загрузку в режиме докачки, например, запускать
команду `curl -OLC- http://…`, прерывая её каждую секунду.
...
* [Рефал-05 с компактным списковым представлением][compact] — ВКР Александра
  ...
  массивов. Выпускная квалификационная работа была выполнена под моим
  руководством, дипломник получил «отлично», хотя производительность не была
  оптимизирована.
